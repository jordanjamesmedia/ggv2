import { Box, Container, Typography, Card, CardContent, CardMedia, CardActionArea, IconButton } from '@mui/material'
...
export default function BlogCarousel({ title = "Latest Articles", excludePostId, maxPosts = 3 }: BlogCarouselProps) {
  ...
  // Filter posts if needed and limit the number
  const filteredPosts = blogPosts
    .filter(post => !excludePostId || post.id !== excludePostId)
    .slice(0, maxPosts)

  const visiblePosts = filteredPosts.slice(startIndex, startIndex + 3)
  const canScrollLeft = startIndex > 0
  const canScrollRight = startIndex + 3 < filteredPosts.length
